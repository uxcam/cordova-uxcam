...
typedef void (^OcclusionFrameRenderingWithOccludeAllTextFieldsHandler)(BOOL occludeAllTextFields, void (^completion)(BOOL status));
/**
...
// Internal Method to handle geture record sent from Flutter
+ (void) handleGestureContent: (CGPoint) position event:(NSDictionary*) event;

/**
 Attach Flutter plugin bridge to UXCam
 @param channel The FlutterMethodChannel to communicate with
 @param basicChannel The FlutterBasicMessageChannel to communicate with
 @return Whether channels are attached or not
 */
+ (BOOL)attachFlutterPluginWithChannel:(id)channel basicChannel:(id)basicChannel;
